...
pub struct Channel {
    topic: Topic,
    members: Vec<Multiaddr>,
    message_cache: Vec<GossipsubMessage>,
    swarm: libp2p::Swarm,
}
            ...
            match swarm.poll_next_unpin(cx) {
                Poll::Ready(Some(event)) => match event {
                    ...
                    GossipsubEvent::Subscribed{peer_id, topic} => {
                        println!("Peer {:?} subscribed to channel {:?}", peer_id, topic);
                    }
                    GossipsubEvent::Unsubscribed{peer_id, topic} => {
                        println!("Peer {:?} unsubscribed from channel {:?}", peer_id, topic)
                    }
                },
